...
<head>
    ...
    <title>{{ $title ?? 'EduCare' }} – Monitoring Risiko Psikososial</title>
    ...
    <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        @php
            $navItems = [
                ...
                ['route' => 'siswa.questionnaire.index','label' => 'Assessment',   'icon' => 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01'],
                ['route' => 'siswa.riwayat.index',      'label' => 'Riwayat',      'icon' => 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z'],
                ['route' => 'siswa.chatbot.index',      'label' => 'EduBot',       'icon' => 'M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z'],
            ];
        @endphp

        @foreach($navItems as $item)
            @php $active = request()->routeIs($item['route']) || (str_contains(request()->route()->getName(), explode('.', $item['route'])[1] ?? ''))  @endphp
...
<div class="lg:pl-64 flex flex-col min-h-screen">
    ...
    <header class="sticky top-0 z-10 bg-white/80 backdrop-blur-md border-b border-slate-100 px-4 sm:px-6 py-3.5 flex items-center gap-4">
        ...
        <div class="flex-1 min-w-0">
            <h1 class="text-base font-semibold text-slate-800 truncate">{{ $pageTitle ?? 'Dashboard' }}</h1>
            @isset($pageSubtitle)
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('siswa.questionnaire.index') }}"
               class="hidden sm:flex items-center gap-2 bg-gradient-to-r from-violet-600 to-indigo-600 text-white px-4 py-2 rounded-xl text-sm font-semibold shadow-sm shadow-violet-200 hover:shadow-violet-300 hover:shadow-md transition-all duration-200">
